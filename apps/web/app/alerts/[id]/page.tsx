...
export default async function AlertDetailPage({
  ...
  const [alertOrNull, user] = await Promise.all([
    apiGet<Alert>(routes.alerts.detail(params.id), 60).catch(() => null),
    getCurrentUser(),
  ]);
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
        ...
        {/* Severity banner for active alerts */}
        {isActive && (
        ...
        <div className="access-note">
          ...
          {user ? (
            ...
            <span>
              ...
              <Link href="/login">sign in</Link> to subscribe to email alerts for this
              {alert.district ? ` district (${alert.district.name})` : ' country'}.
            </span>
          )}
        </div>
      </main>
