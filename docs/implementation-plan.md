...
2. Replace `lib/static-data.ts` calls with `fetch('/api/v1/...')` in Server Components. — **Still partial**: `map-section.tsx`'s "Current conditions" sidebar (2026-08-16) and `metrics-section.tsx`'s four metric cards (2026-08-19) are now live, both with a fallback to static data if the API is unreachable. `dataset-preview`, `reports-alerts-section`, and `biodiversity-restoration`/`community-section` are still fully static — no longer backend-blocked (M9/M10/M11 all shipped real endpoints since this task was scoped), just not yet wired.
3. Add `SWR` or React Query for client-side refreshing data (map, live alerts). — Not needed for the weather or metrics slices done so far: both are Server Components using Next.js's built-in `fetch` cache (`revalidate: 900`) rather than client-side polling. Revisit if a component needs to refresh without a full page reload.
...
- Public page shows real data from the database. — **Still partial**: the weather conditions sidebar and metrics cards are live, plus the nav's session state; `dataset-preview`, `reports-alerts-section`, and `biodiversity-restoration`/`community-section` remain static (task 2).
