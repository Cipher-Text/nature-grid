...
- Connect public web page to live API (replace static seed data) — partially done: the homepage's "Current conditions" sidebar (2026-08-16) and metrics cards (2026-08-19) fetch live data, both with fallback to static data if the API is unreachable; the nav is now session-aware (real login state, 2026-08-16). The dataset preview, reports/alerts preview list, and biodiversity/restoration/community highlight sections are still static — no longer backend-blocked (real endpoints exist for all of them now), just not yet wired.
