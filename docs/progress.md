...
| Prisma schema | Done | 17 enums, 29 models — core entities + 4 weather tables + `FloodForecast` (2026-08-25) + `RefreshToken` + `Observation` + `RestorationProject`/`RestorationParticipant` + `Species`/`Occurrence` + `DatasetAccessRequest` + `AlertSubscription`/`NotificationDelivery` + `ReportComment`/`ReportMedia`; client regenerated |
| Database migration — M4 | Done | 15 migrations applied, 29 tables live (latest: `20260825120000_add_flood_forecasts`, 2026-08-25); Postgres on port 5433 (remapped — local Postgres occupies 5432) |
