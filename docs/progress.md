...
`IngestionModule` was an empty `@Module({})` stub with no service or controller. The `IngestionJob` Prisma model existed and was used by weather/biodiversity schedulers to write `IngestionJob` records — previously skipped with no tracking.
...
18. **Next up:** WAQI integration (M14) for station-level AQI, ingestion observability (job tracking before a 2nd provider), or resume M5's remaining schema items (ReportMedia, ReportComment). `/community` is the only app-shell page left without a real backend, and no milestone plans one yet.
